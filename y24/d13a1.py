...
def parse(itr) -> Machine:
    ...
    p = next(lines).split(", ")
    px = int(p[0][p[0].find('=') + 1:])
    py = int(p[1][p[1].find('=') + 1:])
    return Machine(ax, ay, bx, by, px, py)

def win_comb(m: Machine) -> Optional[tuple[int, int]]:
    denom = m.bx * m.ay - m.ax * m.by
    if denom != 0:
        xa, ra = divmod(m.bx * m.py - m.px * m.by, denom)
        xb, rb = divmod(m.px * m.ay - m.ax * m.py, denom)
        if all(map(lambda rem: rem == 0, (ra, rb))) and \
                all(map(lambda x: 0 <= x <= 100, (xa, xb))):
            return (xa, xb)
    return None
