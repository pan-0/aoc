...
def score2(teaspoons: list[int], ingredients: list[Ingredient]) -> int:
    return \
        0 if propsum(teaspoons, map(lambda ingred: ingred.calories,
                                    ingredients)) != 500 \
        else score(teaspoons, ingredients)
...
def crunch(ingredients: list[Ingredient],
    ...
    if index >= len(teaspoons):
        return max(score2(teaspoons, ingredients), best)
